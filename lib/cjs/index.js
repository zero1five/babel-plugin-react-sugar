...
import { getAndRemoveAttr, randomStr, setAttr, keyAttrBeing, randomStrExpress } from './utils/utils';
  ...
  const visitor = {
    JSXElement(nodePath) {
      ...
      let bindingValue = getAndRemoveAttr(nodePath.node, bindAttrName),
          ...
          ifAttrValue = getAndRemoveAttr(nodePath.node, ifAttrName); // v-model
      ...
      while (loopAttrValue) {
        nodePath.replaceWithMultiple(loopHelper(nodePath, loopAttrValue, loopAttrName));
        loopAttrValue = false;
      } // v-if
    ...
    ArrowFunctionExpression(path, state) {
      ...
      setAttr(path.node.body, t.jSXAttribute(t.jSXIdentifier('key'), t.JSXExpressionContainer(randomStrExpress())));
    },

    ArrayExpression(path, state) {
      if (path.node.elements[0] && path.node.elements[0].type === 'JSXElement') {
        path.node.elements.forEach(element => {
          setAttr(element, t.jSXAttribute(t.jSXIdentifier('key'), t.JSXExpressionContainer(randomStrExpress())));
        });
      }
    }
