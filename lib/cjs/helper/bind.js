...
const extractBindingValue = (jsxExprContainer, attrName) => {
  if (jsxExprContainer.type !== 'JSXExpressionContainer' || jsxExprContainer.expression.type !== 'MemberExpression') {
    throw new TypeError(`Value of attribute [${attrName}] ` + `(line ${jsxExprContainer.loc.start.line}) ` + 'is not a MemberExpression');
  }
...
export const bindHelper = (nodePath, bindingValue, attrName) => {
  ...
  let wrappedExpr = types.callExpression(addDefault(nodePath, path.resolve('./lib/cjs/runtime'), {
    nameHint: 'binding'
